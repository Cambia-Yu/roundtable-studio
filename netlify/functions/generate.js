...
export async function handler(event, context) {
  ...
  if (event.httpMethod !== 'POST') {
    return {
      statusCode: 405,
      headers,
      body: JSON.stringify({ error: '只支持 POST' }),
    };
  }
  ...
  try {
    ...
    if (typeof body === 'string') {
      try {
        ...
        return {
          statusCode: 400,
          headers,
          body: JSON.stringify({ error: '请求体须为 JSON' }),
        };
      }
    }
    ...
    if (!topic) {
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ error: '缺少 topic' }),
      };
    }
    ...
    if (provider === 'anthropic') {
      ...
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ error: 'provider 须为 openai、anthropic 或 zhipu' }),
      };
    }
